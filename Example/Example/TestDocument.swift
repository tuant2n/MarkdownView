...
#### 1. 相交条件
将直线方程 \( y = 2x + 5 \) 和曲线方程 \( y = e^x + x + a \) 相等，得到：
\[
2x + 5 = e^x + x + a
\]
整理后：
...
#### 2. 斜率条件
直线的斜率已知为 2。现在计算曲线的导数，得到其斜率：
\[
y = e^x + x + a \implies y' = e^x + 1
\]
在切点处，曲线的斜率应等于直线的斜率：
\[
e^x + 1 = 2
...
将 \( x = 0 \) 代入相交条件方程 (1)：
\[
e^0 - 0 + a = 5 \implies 1 + a = 5 \implies a = 4
\]

### 验证
当 \( a = 4 \) 时，曲线方程为 \( y = e^x + x + 4 \)。在 \( x = 0 \) 处：
- 曲线的 \( y \) 值：\( y = e^0 + 0 + 4 = 1 + 4 = 5 \)
- 直线的 \( y \) 值：\( y = 2(0) + 5 = 5 \)
- 曲线的导数：\( y' = e^0 + 1 = 1 + 1 = 2 \)，与直线斜率一致。
